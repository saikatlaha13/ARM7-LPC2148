...
{
	LcdData(((num & 0xf0)>>4)+ 0x30);
	LcdData((num & 0x0f)+ 0x30);
}
...
{
		LcdDisplay_BCD_Conv(hour);
		LcdData(':');
		LcdDisplay_BCD_Conv(min);
		LcdData(':');
		LcdDisplay_BCD_Conv(sec);
}
...
{
		LcdDisplay_BCD_Conv(day);
		LcdData('/');
		LcdDisplay_BCD_Conv(month);
		LcdData('/');
		LcdDisplay_BCD_Conv(year);
}
